...
<td><a href=drug.html>Drug eQTL</a></td>
</tr>
</table>
...
//	var plotdata = JSON.parse(JSON.stringify(data[myrow])));
	...
	var sorted = helper.arr.multisort(tobeSorted, ['SNP','IFN'], ['ASC','ASC']);
...
// console.log(JSON.stringify(sorted));
	...
	var xAxisSets = '';
	var yAxisSets = '';
	for (var i=0; i<Alleles_value.length; i++) {
		valStr += '"Tcell_'+Alleles_value[i]+'",'+'"Exp_'+Alleles_value[i]+'",';
		xAxisSets += '"Tcell_'+Alleles_value[i]+'",';
		yAxisSets += '"Exp_'+Alleles_value[i]+'",';
	}
	valStr = valStr.slice(0, -1);
	xAxisSets = xAxisSets.slice(0, -1);
	yAxisSets = yAxisSets.slice(0, -1);
	
	data4plot = data4plot.slice(0, -1);
	data4plot += '}, "y":{ "smps":['+valStr+'], "vars":[';
	...
	for (var i = 0; i < sorted.length; i++) {
		...
		for (var j = 0; j < Alleles_value.length; j++) {
			if (sorted[i]["SNP"] ==  Alleles_value[j]) {
				if (sorted[i]["Tcell"] == "NA") {
	        		valStr += '"NaN",' + valadd.toFixed(3) + ',';
				} else {
	        		valStr += sorted[i]["Tcell"] + ',' + valadd.toFixed(3) + ',';
				}
			} else {
				valStr += '"NaN"' + ',' + '"NaN"' +',';
			}
...
console.log(data4plot);
	...
	params =
		...
		'		   "decorationScaleFontFactor": 2.4,' +
		'			"showLegend": false,' +
        '           "xAxisTitle": "T cell (% of lymphocytes)",' +
        '           "yAxisTitle": "' + gene + ' log2(CPM+1)",' +
		'			"xAxis2Show": false,' +
		'		   "graphType": "Scatter2D",' +
        '          "canvasBox": true,' +
        '          "printType": "window",' +
...
//	afterRenderObject = renderObject.reverse();
	...
	if (afterRenderObject.length < 1) {
		afterRenderObject.push(["addRegressionLine", [] ]);
		afterRenderObject.push(["setDimensions",[480,480]]);
	}


    var cxPlot = new CanvasXpress({
        renderTo: canvasid,
